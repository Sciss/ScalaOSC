...
private[osc] trait BidiImpl extends ChannelImpl with Channel.Bidi {
   protected def input: Channel // SingleInputChannelImpl
   protected def output: Channel // SingleOutputChannelImpl
    ...
    */
   final def dumpIn( mode: Dump, stream: PrintStream, filter: Dump.Filter ) {
    ...
    */
   final def dumpOut( mode: Dump, stream: PrintStream, filter: Dump.Filter ) {
